...
# This script fixes database schema mismatches
...
if [ ! -f "scripts/fix-database-schema.sql" ]; then
    echo "❌ Error: fix-database-schema.sql not found. Please run this script from the project root."
    exit 1
fi
...
# Run the schema fix
docker exec -i gift-tracker-postgres psql -U admin -d gift_tracker < scripts/fix-database-schema.sql

if [ $? -eq 0 ]; then
    echo "✅ Database schema fix completed successfully!"
    echo "🔄 Restarting services to apply changes..."
    
    # Restart the services to pick up the schema changes
    ...
    echo "🎉 Schema fix complete! You can now create new instances."
else
    echo "❌ Database schema fix failed. Please check the error messages above."
    exit 1
fi
